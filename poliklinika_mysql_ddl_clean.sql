...
-- =========================================================
-- NAPOMENA O REFAKTORINGU (po komentarima profesora)
-- =========================================================
-- 1) Status se vise NE prati zasebnim tablicama (sifarnik_statusa_*),
--    nego stupcem `status VARCHAR(30)` + CHECK constraintom u tablici
--    gdje je status relevantan (termin_pacijenta, pregled, nalaz, racun).
-- 2) Tablica `sifarnik_usluga` je obrisana - hijerarhija usluga nije
--    potrebna. `usluga` je sama po sebi dovoljna.
-- 3) `zaposlenik` vise nema `id_odjel` (odjel se dohvaca preko ordinacije).
-- 4) `termin_pacijenta` sada eksplicitno nosi `id_usluga` i `id_ordinacija`.
-- 5) `pregled` vise ne duplicira `id_pacijent` (dolazi preko termina).
--    `id_zaposlenik` se zadrzava jer pregled moze obaviti drugi lijecnik
--    od onog planiranog u terminu - to je svjesno odstupanje radi cuvanja
--    stvarnog stanja pregleda.
-- 6) `nalaz` vise nema `id_zaposlenik`. `id_pacijent` ostaje obavezan
--    (nalaz uvijek pripada pacijentu - moze postojati i bez pregleda,
--    npr. vanjski lab). `id_pregled` je opcionalan.
-- 7) `terapija` vise nema `id_pacijent` ni `id_zaposlenik` - dolaze
--    preko obaveznog `id_pregled`.
-- 8) `stavka_racuna` sada ima FK na `pregled_usluga` (izvor istine za
--    obavljene usluge). Atributi (cijena, popust, porez, iznos) se i
--    dalje kopiraju u stavku jer se izdani racun po zakonu zamrzava
--    u trenutku izdavanja - kasnija promjena pregleda ne smije mijenjati
--    sadrzaj racuna.
-- 9) `privitak_pregleda` zamijenjen opcim modelom `privitak` koji se
--    moze vezati na pregled, nalaz, uputnicu ili racun (CHECK
--    constraint osigurava da je tocno jedan vlasnik postavljen).
-- =========================================================
...
-- Zaposlenik pripada jednoj ordinaciji. Odjel se dohvaca preko ordinacije
-- (ordinacija -> odjel), pa zaposlenik vise NE nosi `id_odjel`.
...
-- `sifarnik_usluga` je uklonjen - usluga je sama po sebi dovoljna.
CREATE TABLE usluga (
...
-- Termin sada eksplicitno nosi za KOJU se uslugu pacijent narucuje
-- i u KOJOJ ordinaciji se termin odrzava. Status je VARCHAR + CHECK
-- (umjesto FK na sifarnik_statusa_termina koji je obrisan).
CREATE TABLE termin_pacijenta (
  ...
  id_usluga BIGINT UNSIGNED NOT NULL,
  id_ordinacija BIGINT UNSIGNED NOT NULL,
  status VARCHAR(30) NOT NULL DEFAULT 'najavljen',
  ...
  KEY idx_termin_pacijenta_usluga_id (id_usluga),
  KEY idx_termin_pacijenta_ordinacija_id (id_ordinacija),
  KEY idx_termin_pacijenta_status (status),
  CONSTRAINT chk_termin_status CHECK (status IN ('najavljen','odrzan','otkazan','nije_odrzan')),
...
-- Pregled je vezan na termin (id_termin_pacijenta) iz kojeg se dohvaca
-- pacijent. `id_pacijent` je obrisan iz pregleda (vise se ne duplicira).
-- `id_zaposlenik` je svjesno ostavljen jer pregled moze obaviti drugi
-- lijecnik od onoga planiranog u terminu (cuvamo stvarno stanje pregleda).
-- Status je VARCHAR + CHECK (nema vise tablice status_pregleda).
CREATE TABLE pregled (
  id_pregled BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  id_termin_pacijenta BIGINT UNSIGNED NOT NULL,
  id_zaposlenik BIGINT UNSIGNED NOT NULL,
  status VARCHAR(30) NOT NULL DEFAULT 'u_tijeku',
  ...
  CONSTRAINT chk_pregled_status CHECK (status IN ('u_tijeku','zavrsen','storniran')),
...
-- pregled_usluga = IZVOR ISTINE za stvarno obavljene usluge na pregledu.
-- stavka_racuna ce kasnije referencirati ovu tablicu.
...
-- Nalaz uvijek pripada pacijentu (id_pacijent NOT NULL). `id_pregled` je
-- opcionalan jer podrzavamo i nalaze koji nisu vezani uz pregled
-- (npr. iz vanjskog laboratorija). `id_zaposlenik` je obrisan - kad
-- nalaz dolazi iz pregleda, lijecnik se dohvaca preko `pregled.id_zaposlenik`.
-- Status je VARCHAR + CHECK (nema vise tablice status_nalaza).
CREATE TABLE nalaz (
  id_nalaz BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  id_pacijent BIGINT UNSIGNED NOT NULL,
  id_tip_nalaza BIGINT UNSIGNED NOT NULL,
  id_pregled BIGINT UNSIGNED NULL,
  status VARCHAR(30) NOT NULL DEFAULT 'u_obradi',
  ...
  KEY idx_nalaz_status (status),
  CONSTRAINT chk_nalaz_status CHECK (status IN ('u_obradi','izdan','ponisten')),
...
-- Terapija uvijek nastaje iz pregleda (id_pregled NOT NULL). Pacijent i
-- lijecnik se dohvacaju preko pregleda - vise ih ne dupliciramo.
...
-- Racun moze postojati i bez pregleda (npr. administrativna naplata),
-- pa `id_pacijent` ostaje izvor istine za pacijenta. `id_pregled` je
-- opcionalna referenca. Status je VARCHAR + CHECK (nema vise tablice
-- status_racuna).
CREATE TABLE racun (
  ...
  id_pacijent BIGINT UNSIGNED NOT NULL,
  id_pregled BIGINT UNSIGNED NULL,
  status VARCHAR(30) NOT NULL DEFAULT 'izdan',
  ...
  KEY idx_racun_status (status),
  CONSTRAINT chk_racun_status CHECK (status IN ('izdan','placen','storniran','dospio')),
...
-- Stavka racuna sada referencira `pregled_usluga` (izvor istine za
-- obavljene usluge). Atributi (cijena, popust, porez, iznos) se i dalje
-- kopiraju u stavku - to je svjesna duplikacija jer je izdani racun
-- po zakonu zamrznut u trenutku izdavanja, neovisno od kasnijih
-- promjena pregleda. `id_pregled_usluga` je NULL-able jer racun moze
-- postojati i bez pregleda (administrativna naplata).
CREATE TABLE stavka_racuna (
  id_stavka_racuna BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  id_racun BIGINT UNSIGNED NOT NULL,
  id_usluga BIGINT UNSIGNED NOT NULL,
  id_pregled_usluga BIGINT UNSIGNED NULL,
  ...
  KEY idx_stavka_racuna_pregled_usluga_id (id_pregled_usluga),
...
-- Opci model privitka. Privitak se moze vezati na pregled, nalaz,
-- uputnicu ILI racun - tocno jedan vlasnik mora biti postavljen
-- (osigurano CHECK constraintom). Ovim je izbjegnuto mnozenje
-- specijaliziranih tablica (privitak_pregleda, privitak_nalaza, ...).
CREATE TABLE privitak (
  id_privitak BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  id_pregled BIGINT UNSIGNED NULL,
  id_nalaz BIGINT UNSIGNED NULL,
  id_uputnica BIGINT UNSIGNED NULL,
  id_racun BIGINT UNSIGNED NULL,
  ...
  KEY idx_privitak_pregled_id (id_pregled),
  KEY idx_privitak_nalaz_id (id_nalaz),
  KEY idx_privitak_uputnica_id (id_uputnica),
  KEY idx_privitak_racun_id (id_racun),
  CONSTRAINT chk_privitak_tocno_jedan_vlasnik CHECK (
    (CASE WHEN id_pregled  IS NULL THEN 0 ELSE 1 END) +
    (CASE WHEN id_nalaz    IS NULL THEN 0 ELSE 1 END) +
    (CASE WHEN id_uputnica IS NULL THEN 0 ELSE 1 END) +
    (CASE WHEN id_racun    IS NULL THEN 0 ELSE 1 END) = 1
  ),
...
-- =========================================================
-- POVEZANOST TABLICA (azurirano nakon refaktoringa)
-- =========================================================
-- pacijent je povezan s: uputnica, termin_pacijenta, nalaz, racun
-- odjel je povezan s: ordinacija, usluga
-- ordinacija je povezan s: zaposlenik, termin_pacijenta
-- specijalizacija je povezan s: zaposlenik
-- zaposlenik je povezan s: raspored_djelatnika, termin_pacijenta, pregled
-- usluga je povezan s: cijena_usluge, termin_pacijenta, pregled_usluga, stavka_racuna
-- uputnica je povezan s: termin_pacijenta, privitak
-- termin_pacijenta je povezan s: pregled
-- pregled je povezan s: pregled_usluga, pregled_dijagnoza, nalaz, terapija, racun, privitak
-- dijagnoza je povezan s: pregled_dijagnoza
-- tip_nalaza je povezan s: nalaz
-- nalaz je povezan s: laboratorijski_rezultat, terapija, privitak
...
-- racun je povezan s: stavka_racuna, uplata, privitak
-- pregled_usluga je povezan s: stavka_racuna
